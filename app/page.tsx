...
// 考勤紀錄 (維持不變，請使用先前的 V5.1 版本，這裡簡化顯示以節省空間)
function AttendanceView() {
  // ... 請將 V5.1 的 AttendanceView 代碼完整貼在這裡 ...
  // 如果需要我再貼一次完整版，請告訴我，但這部分邏輯沒變
  ...
  const fetchRoster = async () => {
    ...
    data?.forEach((r: any) => {
      if(doctors.find(d => d.id === r.staff_id)) {
        // 🔧 關鍵修正：檢查 shifts 是不是陣列，而且裡面的東西是不是物件 (因為有可能存到 "M" 這種字串)
